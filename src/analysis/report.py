...
class ModelAnalysisReport:
    ...
    def add_plot(self, plot_func: Callable[..., Any], heading: str, **kwargs: Any) -> None:
        ...
        # Inspect which arguments the plot function supports
        plot_func_params = inspect.signature(plot_func).parameters
        ...
        for split_value in self.df[self.split_col].unique():
            ...
            # exposure_col is not supported for residual plots if the function supports it
            if self.exposure_col and "exposure_col" in plot_func_params:
                plot_kwargs["exposure_col"] = self.exposure_col
